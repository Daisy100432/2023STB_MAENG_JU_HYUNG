...
table(is.da(welfare23$p_salary))
welfare23$p_salary<-ifelse(welfare23$t_salary==64800,NA,
welfare23$t_salary)
...
#edu
welfare23$edu_grade<-ifelse(welfare23$edu%in%c(1,2,3,4),"중학이하", 
                            ifelse(welfare23$edu==5, "고교",
                                   ifelse(welfare23$edu==6,"전문대","대학 이상")))
...
welfare23%>%
  ...
  select(sex,age,edu_grade,reg,p_salary)
...
ggplot(data = age_salary1, aes(x = age, y = m)) +
  geom_line() +
  geom_point() +
  labs(title = "연령별 평균 총급여 추이", x = "나이", y = "평균 총급여")
